...
(function () {
  ...
  function updateDockUI() {
    ...
    // Synchronize Current Playlist Preview Active Track
    if (typeof PlaylistPreviewEngine !== 'undefined') {
      PlaylistPreviewEngine.updateActiveState();
    }
  }
        ...
        if (playerEl) {
          bgYtPlayer = new window.YT.Player('bg-yt-player', {
            ...
            events: {
              ...
              onError: (err) => {
                console.warn('[Visuals] YouTube background player notice:', err);
              }
            }
          });
        }
      } catch (err) {
        console.warn('[Visuals] YouTube player init notice:', err);
      }
    } else {
      initYouTubeAPI();
    }
  ...
  function updateVisualSoundUI() {
    ...
    const isAudioCapable = state.currentVisual && state.currentVisual !== 'off';
    const isAudible = isAudioCapable && state.visualSoundEnabled && state.visualVolume > 0;

    if (soundRow) {
      soundRow.classList.toggle('is-disabled', !isAudioCapable);
      soundRow.classList.toggle('is-active', isAudible);
    }
    ...
    if (volSlider) {
      ...
      updateSliderTrack(volSlider, state.visualVolume || 0, isAudible);
    }
  ...
  function setVisual(visualId, customUrl = null, persist = true) {
    ...
    document.querySelectorAll('.visual-item-btn').forEach(btn => {
      btn.classList.toggle('is-active', btn.getAttribute('data-visual-id') === preset.id);
    });

    // Update Badges
    const isOff = preset.id === 'off' || preset.type === 'dynamic';
    if (stateBadge) {
      stateBadge.textContent = isOff ? 'Off' : preset.name.split(' ')[0];
      stateBadge.classList.toggle('bg-green-400/20', !isOff);
      stateBadge.classList.toggle('text-green-400', !isOff);
    }
    if (counterBadge) {
      counterBadge.textContent = isOff ? 'Default' : preset.name.split(' ')[0];
    }

    trackEvent('visual_selected', {
      visual_id: preset.id,
      visual_name: preset.name,
  ...
  // ============================================================
  // CURRENT PLAYLIST PREVIEW ENGINE (QUEUE & ACTIVE TRACK INSPECTOR)
  // Connects directly to authoritative state.currentPlaylist and state.tracks
  // ============================================================
  const PlaylistPreviewEngine = (function () {
    let isOpen = false;
    ...
    function render() {
      ...
      const badgeEl = document.getElementById('playlist-preview-badge');
      if (!panel || !listEl) return;

      const currentPlaylist = state.currentPlaylist || { name: 'GullyGang Special' };
      ...
      if (tracks.length === 0) {
        listEl.innerHTML = `
          <li class="playlist-preview-empty">
            <span>No songs loaded in active playlist</span>
          </li>
        `;
        return;
      }
      ...
      const html = tracks.map((track, idx) => {
        ...
        const thumbHtml = artworkUrl ? `
          <img src="${artworkUrl}" class="playlist-preview-thumb" alt="${cleanTitle}" loading="lazy" decoding="async" />
        ` : '';
        ...
        return `
          <li class="playlist-preview-item" role="option" aria-selected="${isActive}">
      ...
      // Attach track selection listeners to rendered rows
      ...
      rowBtns.forEach((btn) => {
        btn.addEventListener('click', (e) => {
          ...
          if (!isNaN(targetIdx) && targetIdx >= 0 && targetIdx < tracks.length) {
            loadTrackAtIndex(targetIdx, true, 'direct');
            updateActiveState();
          }
        });
      });
    ...
    function positionDesktop() {
      ...
      const trigger = document.getElementById('btn-playlist-preview') || document.getElementById('player-right-controls') || document.getElementById('sub-actions-group');
      const viewportWidth = window.innerWidth;
      const viewportHeight = window.innerHeight;
    ...
    function attachListeners() {
      // Global delegation handles all triggers; no per-element re-binding required
    }
    ...
    function init() {
      if (!window.__gullygang_preview_global_attached) {
        ...
        // Global Click Delegator for preview toggle, close, backdrop, and outside click
        document.addEventListener('click', (e) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.getAuthoritativeMusicState = () => ({
  ...
  const GullyRouter = (function () {
    ...
    function rebindPlayerDOM() {
      ...
      PlaylistPreviewEngine.attachListeners();
